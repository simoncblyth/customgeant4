...
custom4_prefix=/data1/blyth/local/custom4_${CUSTOM4_CMAKE_BUILD_TYPE}/$VERSION
...
if [ "${arg/bashrc}" != "$arg" ]; then
   source $REALDIR/generate_bashrc.sh
   echo $BASH_SOURCE generate_bashrc writing to $CUSTOM4_PREFIX/bashrc
   generate_bashrc > $CUSTOM4_PREFIX/bashrc
fi
